...
export default function DrawDetailModal({ draw, onClose, onUpdate }) {
  ...
  const loadImageAsBlob = async () => {
    ...
    try {
      // Backend stores imageUrl as /api/images/filename.png, remove /api/ prefix since NEXT_PUBLIC_API_URL already includes it
      const imageUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:10000/api'}${drawData.imageUrl.replace('/api', '')}`;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          ...
          <button
            onClick={onClose}
            className="ml-2 text-gray-400 hover:text-gray-600 transition p-1"
          >
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          {/* Preselect Section - Only if not totalized */}
          {canPreselect && !drawData.preselectedItemId && drawData.game?.items && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center px-4 py-2.5 bg-blue-600 text-white text-sm sm:text-base rounded-lg hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2 sm:mb-3">Estado del Sorteo</h3>
            ...
            <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2 sm:mb-3">Resultado</h3>
              {drawData.winnerItem ? (
          ...
          {/* Change Preselected - Only if not totalized */}
          {canChangePreselected && drawData.game?.items && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 sm:p-4">
              ...
              <div className="space-y-3">
                ...
                <select
                  value={selectedItemId || ''}
                  onChange={(e) => setSelectedItemId(e.target.value)}
                  className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none"
                >
                ...
                <button
                  ...
                  disabled={preselecting || !selectedItemId}
                  className="w-full flex items-center justify-center px-4 py-2.5 bg-orange-600 text-white text-sm sm:text-base rounded-lg hover:bg-orange-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <div className="flex flex-wrap gap-2 sm:flex-nowrap">
                  {drawData.imageUrl && (drawData.imageGenerated || imageExists) ? (
                    <>
                      {!imageLoadError && (
                        <button
                          ...
                          className="flex items-center px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
                      ...
                      {imageLoadError && (
                        <button
                          ...
                          className="flex items-center px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition"
                      ...
                      <button
                        ...
                        className="flex items-center px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition disabled:opacity-50"
                        title="Regenerar imagen"
                      >
                        <RefreshCw className={`w-4 h-4 sm:mr-1 ${generatingImage ? 'animate-spin' : ''}`} />
                    ...
                    <button
                      ...
                      disabled={generatingImage}
                      className="flex items-center px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50"
              ...
              <div className="space-y-2 sm:space-y-3">
                {drawData.publications.map((pub) => {
                  ...
                  return (
                    <div
                      key={pub.id}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 bg-white rounded-lg border border-gray-200 gap-2 sm:gap-0"
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-8 h-8 sm:w-10 sm:h-10 ${statusInfo.bg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                          <StatusIcon className={`w-4 h-4 sm:w-5 sm:h-5 ${statusInfo.color}`} />
                        </div>
                        <div className="min-w-0">
                          <div className="font-medium text-gray-900 text-sm sm:text-base">{pub.channel}</div>
                          <div className="text-xs sm:text-sm text-gray-600">{statusInfo.label}</div>
                          {pub.sentAt && (
                            <div className="text-xs text-gray-500">
                      ...
                      {(pub.status === 'FAILED' || pub.status === 'PENDING') && (
                        <button
                          ...
                          className="flex items-center justify-center px-3 py-1.5 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition disabled:opacity-50 self-end sm:self-auto"
                        >
          ...
          {drawData.notes && (
            <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
              ...
              <p className="text-xs sm:text-sm text-gray-600">{drawData.notes}</p>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end p-4 sm:p-6 border-t border-gray-200 sticky bottom-0 bg-white">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-100 text-gray-700 text-sm sm:text-base rounded-lg hover:bg-gray-200 transition"
          >
            Cerrar
          </button>
        </div>
