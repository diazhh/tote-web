...
/**
 * Componente para gestionar instancias de Instagram
 */
export default function InstagramInstanceManager() {
  ...
  const loadInstances = async (forceRefresh = false) => {
    try {
      ...
      const data = await instagramAPI.listInstances();
  ...
  const createInstance = async () => {
    try {
      const result = await instagramAPI.createInstance({
        instanceId: newInstance.instanceId,
        name: newInstance.name,
        appId: newInstance.appId,
        appSecret: newInstance.appSecret,
        redirectUri: newInstance.redirectUri
      });

      toast.success('Instancia de Instagram creada. Autoriza la aplicación.');
  ...
  const authorizeInstance = async () => {
    try {
      await instagramAPI.authorizeInstance(
        selectedInstance.instanceId,
        authCode,
        selectedInstance.config?.redirectUri
      );

      toast.success('Instagram autorizado exitosamente');
  ...
  const refreshToken = async (instanceId) => {
    try {
      const result = await instagramAPI.refreshToken(instanceId);
  ...
  const testConnection = async (instanceId) => {
    try {
      const result = await instagramAPI.testConnection(instanceId);
  ...
  const disconnectInstance = async (instanceId) => {
    ...
    try {
      await instagramAPI.disconnectInstance(instanceId);
  ...
  const deleteInstance = async (instanceId) => {
    ...
    try {
      await instagramAPI.deleteInstance(instanceId);
  ...
  const toggleActive = async (instance) => {
    try {
      ...
      await instagramAPI.toggleActive(instance.instanceId, newStatus);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Instancias de Instagram</h2>
          <p className="text-gray-600 mt-1">Gestiona las conexiones de Instagram Basic Display API</p>
        </div>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    ...
                    <p><strong>App ID:</strong> {instance.appId}</p>
                    {instance.username && (
                      <p><strong>Usuario:</strong> @{instance.username}</p>
                    )}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold mb-4">Nueva Instancia de Instagram</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Instagram Principal"
                />
              </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                disabled={!newInstance.name || !newInstance.instanceId || !newInstance.appId || !newInstance.appSecret || !newInstance.redirectUri}
      ...
      {showAuthModal && selectedInstance && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold mb-4">Autorizar Instagram</h3>
            
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                Para completar la autorización, visita la URL de Instagram y copia el código de autorización:
              </p>
              
              <div className="bg-gray-50 p-3 rounded-lg">
                <p className="text-xs text-gray-500 mb-2">URL de autorización:</p>
                <a 
                  href={instagramAPI.generateAuthUrl(selectedInstance.appId, selectedInstance.config?.redirectUri)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 hover:text-blue-700 text-sm break-all"
                >
                  {instagramAPI.generateAuthUrl(selectedInstance.appId, selectedInstance.config?.redirectUri)}
                </a>
              </div>
