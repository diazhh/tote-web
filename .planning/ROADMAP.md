...
Plans:
- [ ] 14-01-PLAN.md — Data preparation: NEW backfill-td-drawid.mjs (P-D mitigation, ~621k TicketDetail.drawId NULL fix) + NEW fix-prizes-processed.mjs (D-05 step 1, ~5804 historical Draw rows) + re-run Phase 11 backfill-draw-financials.mjs against the local mirror (LOCAL-ONLY); REPORT_USE_MATERIALIZED stays disabled at end
- [ ] 14-02-PLAN.md — Service refactor: monitor.service.js + accounting-report.service.js branched on useMaterialized with legacy body byte-equivalent (P-A snapshot fixture); resolveApiSystemFilter helper extracted (O4); draw.service.js#getDrawById extended with financial + financialProviders includes (O3 — FIN-REPORT-04 backend); shadow-comparison Jest test (D-06) demonstrates multi-draw bug closes; REPORT_USE_MATERIALIZED=true set in backend/.env in the FINAL task only after all tests pass
- [ ] 14-03-PLAN.md — P&L backend: NEW pnl-report.service.js with getWeeklyPnl(formula = income − prizes − commissions − expenses, D-02), buildPnlExcel (SUM-formula auditable), buildPnlPdf (drawTable helper) + controller + routes mounted at /api/reportes/pnl/semanal{,/excel,/pdf}; dateUtils.js gains getMondayOfISOWeek + re-exports getISOWeek/getISOWeekYear; tests: FIN-REPORT-05 formula, P-B PAYMENT double-count guard, P-C empty-week zero-row, FIN-REPORT-07 Excel formula + PDF magic
- [ ] 14-04-PLAN.md — Frontend + deploy doc: NEW /admin/reportes/pnl-semanal page (filters, drill-down to comisiones/contabilidad via ?week=YYYY-Www, Excel + PDF download); extend DrawDetailModal.js (admin path — not draws/) with Financiero section reading drawData.financial + drawData.financialProviders; sidebar entry; backend/.env.example documents REPORT_USE_MATERIALIZED with default false + 3-gate condition; 14-DEPLOY.md production runbook (LOCAL-ONLY this session); [CHECKPOINT] BLOCKING human-verify with milestone v1.3 closeout
