...
class PrewinnerOptimizerService {
  ...
  /**
   * Pesos para cada criterio de scoring (v3 - más peso a tripletas)
   */
  static WEIGHTS = {
    DAYS_SINCE_WIN: 0.25,      // Tiempo sin ganar (historial real)
    SEQUENTIAL_PENALTY: 0.20,  // Evitar secuencias (incluyendo día anterior)
    RECENCY_PENALTY: 0.15,     // Penalizar ganadores muy recientes
    TICKET_COUNT: 0.15,        // Maximizar tickets ganadores
    TRIPLETA_RISK: 0.25        // Minimizar riesgo de tripletas (AUMENTADO)
  };
  ...
  static DEFAULTS = {
    ...
    MAX_DAYS_BONUS: 10,             // Días máx para normalizar score (reducido para más sensibilidad)
    ...
    TRIPLETA_PARTIAL_PENALTY: 0.3   // Penalización por tripletas parciales (1 número ya salió)
  };
  ...
  async evaluateCandidates(context, constraints, history) {
    const { gameItems, salesByItem, usedItemsToday, usedCentenasToday, usedItemsLast7Days, activeTripletas, draw } = context;
    const candidates = [];
    const now = new Date();
    ...
    for (const item of gameItems) {
      ...
      // 6. No puede causar pérdidas (pagar más de lo vendido)
      if (totalPayout > context.totalSales) {
        continue;
      }

      // 7. RESTRICCIÓN DURA DE TRIPLETAS: Si completaría AL MENOS 1 tripleta, BLOQUEAR (política agresiva)
      ...
      // v2: Días desde última victoria usando HISTORIAL REAL
      ...
      const daysSinceWin = lastWinInfo 
        ...
        : 365; // Si nunca ha ganado en historial, dar un valor alto
   ...
   * v2: Agrega score de recencia y mejora secuencias
   */
  calculateScores(item, sales, totalPayout, tripletaImpact, daysSinceWin, history, constraints, context) {
    ...
    // v2: Si está en período de cooldown, penalización fuerte
    if (daysSinceWin < cooldownDays && daysSinceWin < 365) {
      // Dentro del cooldown: score muy bajo (0.05 a 0.15)
      scores.recency = 0.05 + (daysSinceWin / cooldownDays) * 0.1;
    } else if (daysSinceWin < cooldownDays + 3) {
      // Justo después del cooldown: score medio (0.3 a 0.6)
      scores.recency = 0.3 + ((daysSinceWin - cooldownDays) / 3) * 0.3;
    } else {
      // Fuera del cooldown: score alto
      scores.recency = Math.min(1, 0.6 + (daysSinceWin - cooldownDays - 3) / 10);
    }

    // 5. TRIPLETA_RISK - Penalizar items que completan tripletas o están en tripletas parciales
    // v3: Considera tanto tripletas que se completarían como tripletas parciales (riesgo futuro)
    let tripletaScore = 1.0;
    
    if (tripletaImpact.completedCount > 0) {
      // Tripletas que se completarían - penalización muy fuerte
      const tripletaRatio = tripletaImpact.totalPrize / (constraints.maxPayout || 1);
      tripletaScore = Math.max(0, 1 - tripletaRatio * 2); // Puede llegar a 0
    } else if (tripletaImpact.count > 0) {
      // Tripletas parciales (1 número ya salió) - penalización moderada
      ...
      // Más penalización si hay tripletas donde solo falta 1 número
  ...
  /**
   * Calcular score final ponderado
   * v2: Usa nuevos pesos con recency
   */
  ...
  async selectRandomIntelligent(context) {
    const { gameItems, usedItemsToday, usedCentenasToday, usedItemsLast7Days, game, draw } = context;
    const history = await this.getDrawHistory(context);
    const cooldownDays = PrewinnerOptimizerService.DEFAULTS.COOLDOWN_DAYS;
